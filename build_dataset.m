...
for i = 1:n_files
    ...
    file_path = fullfile('data/raw/txt', file_name);
    ...
    for idx = 1:size(matching_activations, 1)
        ...
        if contains(file_name, 'open')
            features_open = [features_open; combined_features];
        elseif contains(file_name, 'close')
            features_close = [features_close; combined_features];
...
all_features = [features_open; features_close; features_ok; features_point; features_pinch];

% Create a label array for each gesture type
labels_open = repmat("open", size(features_open, 1), 1); 
    % Creates a string array of labels 'open'. 
    % size(features_open, 1) returns the number of rows ('1') in the
    % features_open matrix.
...
labels_close = repmat("close", size(features_close, 1), 1);
...
all_labels = [labels_open; labels_close; labels_ok; labels_point; 
    labels_pinch];
...
% Split the shuffled dataset into deterministic training and testing sets
...
% Save datasets in MATLAB format
save(fullfile(processed_path, 'dataset.mat'), 'data_table', 'train_data', 'test_data');

% Save datasets in CSV format
writetable(data_table, fullfile(processed_path, 'full_dataset.csv'));
writetable(train_data, fullfile(processed_path, 'train_data.csv'));
writetable(test_data, fullfile(processed_path, 'test_data.csv'));